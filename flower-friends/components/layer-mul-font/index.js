...
import { V2, getImageInfo, Layer, contain, ArrangeType, AlignType, measureOneLineText, base64ToTempFilePath } from '../layer/index'
...
Component({
  ...
  methods: {
    ...
    async render() {
      ...
      const { canvasWidth, width, canvasHeight, bgColor, fontColor, fontFamily, fontSize, letterSpace, lineSpace, text, fontAlign } = this.data;
      ...
      if (fontAlign === AlignType.left) {
        ...
        let r = text;
        while (r.length > 0) {
          let ol = await measureOneLineText(ctx, r, width, letterSpace)
