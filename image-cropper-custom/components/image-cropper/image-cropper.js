import { aspectFit, aspectFill } from './image-mode'
const ImageMode = {
    aspectFit,
    aspectFill
}
Component({
    ...
    data: {
        ...
        flag_img_endtouch: true, // 是否结束触摸(用于边框展示)
    ...
    methods: {
        /**
         * 初始化图片
         * @param {'aspectFit'|'aspectFill'} mode
         * aspectFit	缩放模式，保持纵横比缩放图片，使图片的长边能完全显示出来。也就是说，可以完整地将图片显示出来。
         * aspectFill	缩放模式，保持纵横比缩放图片，只保证图片的短边能完全显示出来。也就是说，图片通常只在水平或垂直方向是完整的，另一个方向将会发生截取。
         */
        async initImg(mode = 'aspectFill') {
            /** @type {import('./image-utils').ImageInfoRes} */
            const imageObject = this.data.imageObject
            if (imageObject) {
                const { offsetX, offsetY, resultWidth, resultHeight } = ImageMode[mode](imageObject.width, imageObject.height, this.data.width, this.data.height);
                this.setData({
                    _img_left: offsetX,
                    _img_top: offsetY,
                    img_width: resultWidth,
                    img_height: resultHeight,
                })
            }
        },
        /**
        ...
        getImg(getCallback) {
            this._draw(() => {
                ...
                // console.log(this.data.export_scale)
                // console.log(this.data.height)
                console.log(this.data.export_scale)
                console.log(this.data.width * this.data.export_scale)
                console.log(this.data.height * this.data.export_scale)
         ...
         */
        setCutCenter() {
            var num = this.data.culNum
            var obj = this.data.info;
            if (num == 0) {
                var str = wx.getSystemInfoSync();
                str.windowHeight = str.windowHeight - this.data.pdbper;
                this.setData({
                    info: str
                })
                wx.showLoading({
        ...
        imgReset() {
            this.setData({
                scale: 1,
                angle: 0,
                _img_top: this.data.info.windowHeight / 2,
        ...
        _start(event) {
            ...
            this.setData({ flag_img_endtouch: false })
        ...
        _end(event) {
            ...
            this.setData({ flag_img_endtouch: true })
            this._moveStop();
        },
        //点击中间剪裁框处理
        _click(event) {
